classdef ActChannels < handle
    ...
    methods
        function self=ActChannels(channelstype, dt, Vm)
            ...
            self.fchan = cell(length(channelstype),1);
            self.ch_inf = cell(length(channelstype),2);% openning/closing
            self.ch_tau = cell(length(channelstype),2);% openning/closing
